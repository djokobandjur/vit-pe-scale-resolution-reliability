"""
SCRIPT 02 — Attention Entropy across layers
============================================

Run in Colab:
    %run /content/02_attention_entropy.py

Output: results/table1/attention_entropy.json
"""
...
SEEDS      = [42, 123, 456]
MID_LAYERS = list(range(4, 9))   # 0-indexed layers 4-8 = human layers 5-9
N_BATCHES  = 20
BATCH_SIZE = 128
...
def make_cifar_loader():
    # No Resize — CIFAR-100 is natively 32x32
    tf = transforms.Compose([
        transforms.ToTensor(),
...
def detect_arch(state: dict):
    """Infer patch_size and img_size from checkpoint weights."""
...
# ── Entropy computation ───────────────────────────────────────────────────────
def entropy_from_attn(attn_w: torch.Tensor) -> float:
    """
    Compute mean Shannon entropy of attention distributions (in nats).
    attn_w: [B, N, N] or [B, H, N, N]
    """
    if attn_w.dim() == 4:
        attn_w = attn_w.mean(dim=1)   # average over heads → [B, N, N]
    eps = 1e-9
    H   = -(attn_w * (attn_w + eps).log()).sum(dim=-1)   # [B, N]
    return H.mean().item()

@torch.no_grad()
def compute_attention_entropy(model, loader, n_batches):
    """
    Manually pass through each block with return_attention=True
    to obtain attention weights without monkey-patching the model.
    Returns {layer_idx: mean_entropy} for all layers where attention is available.
    """
    layer_entropy = {i: [] for i in range(len(model.blocks))}
            ...
            try:
                ...
                if isinstance(out, tuple):
                    ...
                    if attn_w is not None:
                        layer_entropy[i].append(entropy_from_attn(attn_w))
                else:
                    x = out
            except TypeError:
                # block.forward does not accept return_attention — standard pass
                x = block(x)

    return {i: float(np.mean(vals)) for i, vals in layer_entropy.items() if vals}

def mid_layer_entropy(layer_means, mid_layers):
    """Average entropy over the specified mid-layers."""
    vals = [layer_means[l] for l in mid_layers if l in layer_means]
...
for ds_name, results_root, loader_fn, n_classes in configs:
    ...
    for pe in PE_TYPES:
        ...
        for seed in SEEDS:
            ...
            layer_means = compute_attention_entropy(model, loader, N_BATCHES)

            if not layer_means:
                print("NO ATTN WEIGHTS — check block.forward signature")
                per_seed[seed] = None
            else:
                ent = mid_layer_entropy(layer_means, MID_LAYERS)
                per_seed[seed] = round(ent, 4)
                vals.append(ent)
                print(f"{ent:.4f} nats  (layers captured: {sorted(layer_means.keys())})")
        ...
        if vals:
            ...
            output[ds_name][pe] = {
                ...
                'mean':            round(mu, 4),
                'std':             round(sigma, 4),
                'mid_layers_used': MID_LAYERS,
            }
            seeds_str = "  ".join(
                f"{per_seed[s]:8.4f}" if per_seed.get(s) else "     N/A"
                for s in SEEDS
            )
...
# ── Save results ──────────────────────────────────────────────────────────────
out_path = os.path.join(OUT_DIR, 'attention_entropy.json')

# Merge with existing file if present (e.g. IN-100 from a previous run)
...
for ds_label, ds_key in [('IN-100', 'imagenet100'), ('C-100', 'cifar100')]:
    row = f"{'Attn. Entropy (nats)':<28} {ds_label:<10}"
    for pe in PE_TYPES:
        d    = output.get(ds_key, {}).get(pe)
        row += f"  {d['mean']:.4f}±{d['std']:.4f}  " if d else f"  {'N/A':^13}  "
    print(row)
print(f"\n  (mid layers = {MID_LAYERS}, N_BATCHES = {N_BATCHES})")
