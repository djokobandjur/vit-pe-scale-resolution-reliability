...
*   Open `reproduce_paper_results.ipynb` from your Google Drive.
*   **Runtime:** Select **GPU (H100 or A100)** via *Runtime > Change runtime type*.
*   **Mount Drive:** Run the first cell and click **"Connect to Google Drive"** when prompted to allow access.


### **Step 4 --- Code Execution**
*   Run the cells sequentially from top to bottom (or use **Run all** / Ctrl+F9).
*   **Stay Active:** If using "Run all", stay on the tab for the first 30 seconds to confirm the Drive access prompt, otherwise the execution will pause.
*   **Automatic Saving:** The scripts will automatically create the `figures/`, `table1/`, and `table3/` folders within `/pe_experiment/results/` and **save/store all generated outputs directly into them.**
